...
// OptimizePatch takes a patch and rewrites it in optimized form
func OptimizePatch(oldPath string, oldFilter tlc.FilterFunc, newPath string, newFilter tlc.FilterFunc, reader eos.File, writer io.Writer) (err error) {
	oldContainer, err := tlc.WalkDir(oldPath, tlc.WalkOpts{Filter: oldFilter})
	if err != nil {
		return errors.Wrapf(err, "walking %v as directory", oldPath)
	}
	oldPool := fspool.New(oldContainer, oldPath)
	...
	rc, err := rediff.NewContext(rediff.Params{
		...
		PatchReader:           seeksource.FromFile(reader),
	...
	if err != nil {
		return errors.Wrap(err, "creating the patch optimization context")
	}
	...
	if err != nil {
		return errors.Wrap(err, "optimizing patch")
	}
